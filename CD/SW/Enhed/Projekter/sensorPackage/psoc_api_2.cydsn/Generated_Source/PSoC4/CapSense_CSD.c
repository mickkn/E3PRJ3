...
/* SmartSense functions */
	...
	extern uint8 scanSpeedTbl[((CapSense_CSD_TOTAL_SENSOR_COUNT - 1u) / 8u) + 1u];
...
uint8 CapSense_CSD_unscannedSnsDriveMode[CapSense_CSD_TOTAL_SENSOR_COUNT];
...
{   
	...
	{
		CapSense_CSD_unscannedSnsDriveMode[curSensor] = CapSense_CSD_CONNECT_INACTIVE_SNS;
	}
...
void CapSense_CSD_Start(void)
{
    ...
    /* AutoTunning start */
    #if ((CapSense_CSD_TUNING_METHOD == CapSense_CSD__TUNING_AUTO) &&\
		 (0u != CapSense_CSD_CSHL_API_GENERATE))
        ...
        CapSense_CSD_lowLevelTuningDone = 1u;
			...
			for(curSensor = 0u; curSensor < CapSense_CSD_TOTAL_SCANSLOT_COUNT; curSensor++)
			{
				...
				for(curSensor = 0u; curSensor < CapSense_CSD_TOTAL_SCANSLOT_COUNT; curSensor++)
			    {
				    CapSense_CSD_compensationIDAC[curSensor] = (CapSense_CSD_modulationIDAC[curSensor] + 1u) / 2u;
					CapSense_CSD_modulationIDAC[curSensor] = CapSense_CSD_modulationIDAC[curSensor] / 2u;
			    }
...
__INLINE void CapSense_CSD_DisableSensor(uint32 sensor)
{
    ...
    *CapSense_CSD_prtSelTbl[port] &= ~(CapSense_CSD_CSD_HSIOM_MASK << pinHSIOMShift);
	    ...
	    if(sensorState == CapSense_CSD__SHIELD)
		{
			sensorState = CapSense_CSD__GROUND;
		}
	#endif /* (CapSense_CSD_IS_SHIELD_ENABLE == 0) */
	...
	if(sensorState != (uint32)CapSense_CSD__SHIELD)
	{
		...
		{   
		    /* Connected to Ground */
		    inactiveConnect = CapSense_CSD_SNS_GROUND_CONNECT;
		}
...
void CapSense_CSD_PreScan(uint32 sensor)
{
	...
	#if ((CapSense_CSD_TUNING_METHOD == CapSense_CSD__TUNING_AUTO) &&\
		 (0 != CapSense_CSD_IS_OVERSAMPLING_EN))
	    oversamplingFactor = CapSense_CSD_GetBitValue(scanSpeedTbl, sensor);
