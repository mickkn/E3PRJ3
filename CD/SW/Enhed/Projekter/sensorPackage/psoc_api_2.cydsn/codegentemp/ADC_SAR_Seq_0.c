...
***************************************/
uint8 ADC_SAR_Seq_0_initVar = 0u;
volatile int16 ADC_SAR_Seq_0_offset[ADC_SAR_Seq_0_TOTAL_CHANNELS_NUM];
volatile int32 ADC_SAR_Seq_0_countsPer10Volt[ADC_SAR_Seq_0_TOTAL_CHANNELS_NUM];   /* Gain compensation */
...
static const uint32 CYCODE ADC_SAR_Seq_0_channelsConfig[] = { 0x00000000u };


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  The ADC_SAR_Seq_0_initVar variable is used to indicate when/if initial
*  configuration of this component has happened. The variable is initialized to
*  zero and set to 1 the first time ADC_Start() is called. This allows for
*  component Re-Start without re-initialization in all subsequent calls to the
*  ADC_SAR_Seq_0_Start() routine.
*  If re-initialization of the component is required the variable should be set
*  to zero before call of ADC_SAR_Seq_0_Start() routine, or the user may call
*  ADC_SAR_Seq_0_Init() and ADC_SAR_Seq_0_Enable() as done in the
*  ADC_SAR_Seq_0_Start() routine.
*
*******************************************************************************/
void ADC_SAR_Seq_0_Start(void)
{
    ...
    if(ADC_SAR_Seq_0_initVar == 0u)
    {
        ADC_SAR_Seq_0_Init();
        ADC_SAR_Seq_0_initVar = 1u;
    }
    ADC_SAR_Seq_0_Enable();
}
...
{
    ADC_SAR_Seq_0_SAR_CTRL_REG |= ADC_SAR_Seq_0_ENABLE;
    /* The block is ready to use 10 us after the enable signal is set high. */
    CyDelayUs(ADC_SAR_Seq_0_10US_DELAY);
}


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_Stop
********************************************************************************
*
* Summary:
...
{
    ADC_SAR_Seq_0_SAR_CTRL_REG &= (uint32)~ADC_SAR_Seq_0_ENABLE;
}


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_StartConvert
********************************************************************************
*
* Summary:
*  Description:
...
void ADC_SAR_Seq_0_StartConvert(void)
{
        ...
        ADC_SAR_Seq_0_SAR_SAMPLE_CTRL_REG |= ADC_SAR_Seq_0_CONTINUOUS_EN;
    #else /* Firmware trigger */
        ADC_SAR_Seq_0_SAR_START_CTRL_REG = ADC_SAR_Seq_0_FW_TRIGGER;
...
/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_StopConvert
********************************************************************************
*
* Summary:
...
{
    #if(ADC_SAR_Seq_0_DEFAULT_SAMPLE_MODE_SEL == ADC_SAR_Seq_0__FREERUNNING)
        ADC_SAR_Seq_0_SAR_SAMPLE_CTRL_REG &= (uint32)(~ADC_SAR_Seq_0_CONTINUOUS_EN);
    #endif /* ADC_SAR_Seq_0_DEFAULT_SAMPLE_MODE == ADC_SAR_Seq_0__FREERUNNING */
}


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_IsEndConversion
********************************************************************************
*
* Summary:
...
uint32 ADC_SAR_Seq_0_IsEndConversion(uint32 retMode)
{
    ...
    if((retMode & (ADC_SAR_Seq_0_RETURN_STATUS | ADC_SAR_Seq_0_WAIT_FOR_RESULT)) != 0u)
    {
        do
        {
            status = ADC_SAR_Seq_0_SAR_INTR_REG & ADC_SAR_Seq_0_EOS_MASK;
        }while((status == 0u) && ((retMode & ADC_SAR_Seq_0_WAIT_FOR_RESULT) != 0u));

        if(status != 0u)
        {
            /* Clear EOS bit */
            ADC_SAR_Seq_0_SAR_INTR_REG = ADC_SAR_Seq_0_EOS_MASK;
        }
    }

    #if(ADC_SAR_Seq_0_INJ_CHANNEL_ENABLED)
        if((retMode & (ADC_SAR_Seq_0_RETURN_STATUS_INJ | ADC_SAR_Seq_0_WAIT_FOR_RESULT_INJ)) != 0u)
        {
            do
            {
                status |= ADC_SAR_Seq_0_SAR_INTR_REG & ADC_SAR_Seq_0_INJ_EOC_MASK;
            }while(((status & ADC_SAR_Seq_0_INJ_EOC_MASK) != 0u) &&
                   ((retMode & ADC_SAR_Seq_0_WAIT_FOR_RESULT_INJ) != 0u));

            if((status & ADC_SAR_Seq_0_INJ_EOC_MASK) != 0u)
            {
                /* Clear Injection EOS bit */
                ADC_SAR_Seq_0_SAR_INTR_REG = ADC_SAR_Seq_0_INJ_EOC_MASK;
            }
        }
    #endif /* ADC_SAR_Seq_0_INJ_CHANNEL_ENABLED */
...
/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_SetChanMask
********************************************************************************
*
* Summary:
...
{
    ADC_SAR_Seq_0_SAR_CHAN_EN_REG = mask & ADC_SAR_Seq_0_MAX_CHANNELS_EN_MASK;
}
    ...
    /*******************************************************************************
    * Function Name: ADC_SAR_Seq_0_EnableInjection
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        ADC_SAR_Seq_0_SAR_INJ_CHAN_CONFIG_REG |= ADC_SAR_Seq_0_INJ_CHAN_EN;
    }
...
/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_SetLowLimit
********************************************************************************
*
* Summary:
...
{
    ADC_SAR_Seq_0_SAR_RANGE_THRES_REG &= (uint32)(~ADC_SAR_Seq_0_RANGE_LOW_MASK);
    ADC_SAR_Seq_0_SAR_RANGE_THRES_REG |= lowLimit & ADC_SAR_Seq_0_RANGE_LOW_MASK;
}


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_SetHighLimit
********************************************************************************
*
* Summary:
*  Sets the low limit parameter for a limit condition.
...
{
    ADC_SAR_Seq_0_SAR_RANGE_THRES_REG &= (uint32)(~ADC_SAR_Seq_0_RANGE_HIGH_MASK);
    ADC_SAR_Seq_0_SAR_RANGE_THRES_REG |= (uint32)(highLimit << ADC_SAR_Seq_0_RANGE_HIGH_OFFSET);
}


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_SetLimitMask
********************************************************************************
*
* Summary:
...
{
    ADC_SAR_Seq_0_SAR_RANGE_INTR_MASK_REG = mask & ADC_SAR_Seq_0_MAX_CHANNELS_EN_MASK;
}


/*******************************************************************************
* Function Name: ADC_SAR_Seq_0_SetSatMask
********************************************************************************
*
* Summary:
...
{
    ADC_SAR_Seq_0_SAR_SATURATE_INTR_MASK_REG = mask & ADC_SAR_Seq_0_MAX_CHANNELS_EN_MASK;
}
...
void ADC_SAR_Seq_0_SetGain(uint32 chan, int32 adcGain)
{
    ...
    /*******************************************************************************
    * Function Name: ADC_SAR_Seq_0_CountsTo_mVolts
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *  ADC_SAR_Seq_0_countsPer10Volt:  used to convert ADC counts to mVolts.
    ...
    *******************************************************************************/
    int16 ADC_SAR_Seq_0_CountsTo_mVolts(uint32 chan, int16 adcCounts)
    {
        ...
        mVolts = (int16)((((int32)adcCounts * ADC_SAR_Seq_0_10MV_COUNTS) + ( (adcCounts > 0) ? 
                 (ADC_SAR_Seq_0_countsPer10Volt[chan] / 2) : (-(ADC_SAR_Seq_0_countsPer10Volt[chan] / 2)) )) 
                 / ADC_SAR_Seq_0_countsPer10Volt[chan]);
    ...
    /*******************************************************************************
    * Function Name: ADC_SAR_Seq_0_CountsTo_uVolts
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *  ADC_SAR_Seq_0_countsPer10Volt:  used to convert ADC counts to uVolts.
    *  ADC_SAR_Seq_0_Offset:  Used as the offset while converting ADC counts 
    *   to mVolts.
    *
    * Theory: 
    ...
    *******************************************************************************/
    int32 ADC_SAR_Seq_0_CountsTo_uVolts(uint32 chan, int16 adcCounts)
    {
        ...
        uVolts = ((int64)adcCounts * ADC_SAR_Seq_0_10UV_COUNTS) / ADC_SAR_Seq_0_countsPer10Volt[chan];
